...
// POST /api/bookings/duplicate-all?year=2026 — barcha bookinglarni keyingi yilga nusxalash
router.post('/duplicate-all', authenticate, async (req, res) => {
  try {
    const year = parseInt(req.query.year) || new Date().getFullYear();
    const nextYear = year + 1;

    const originals = await prisma.booking.findMany({
    ...
    for (const original of originals) {
      const existing = await prisma.booking.findFirst({
        where: { bookingNumber: original.bookingNumber, bookingYear: nextYear }
      });
      if (existing) {
        skipped++;
        skippedNumbers.push(original.bookingNumber);
        continue;
      }
      await prisma.booking.create({
        data: {
          bookingNumber:   original.bookingNumber,
          bookingYear:     nextYear,
          tourTypeId:      original.tourTypeId,
          departureDate:   shiftYear(original.departureDate),
          arrivalDate:     shiftYear(original.arrivalDate),
          endDate:         shiftYear(original.endDate),
          pax:             0,
          paxUzbekistan:   null,
          paxTurkmenistan: null,
          country:         original.country,
          guideId:         original.guideId,
          secondGuideId:   original.secondGuideId,
          trainTickets:    original.trainTickets,
          avia:            original.avia,
          roomsDbl:        original.roomsDbl,
          roomsTwn:        original.roomsTwn,
          roomsSngl:       original.roomsSngl,
          roomsTotal:      original.roomsTotal,
          dateOlot:        shiftYear(original.dateOlot),
          dateJartepa:     shiftYear(original.dateJartepa),
          dateOybek:       shiftYear(original.dateOybek),
          dateChernyaevka: shiftYear(original.dateChernyaevka),
    ...
    res.json({ created, skipped, skippedNumbers, nextYear });
  } catch (error) {
    console.error('Duplicate all error:', error);
    res.status(500).json({ error: 'Nusxalashda xatolik' });
  }
});

// POST /api/bookings/:id/duplicate — nusxa yaratish (keyingi yil uchun)
router.post('/:id/duplicate', authenticate, async (req, res) => {
  try {
    ...
    // Shift all dates by exactly 1 year
    const shiftYear = (date) => {
      if (!date) return null;
      const d = new Date(date);
      d.setFullYear(d.getFullYear() + 1);
    ...
    const newDepartureDate = shiftYear(original.departureDate);
    const newBookingYear = newDepartureDate.getFullYear();

    // Check uniqueness
    const existing = await prisma.booking.findFirst({
      where: { bookingNumber: original.bookingNumber, bookingYear: newBookingYear }
    });
    if (existing) {
      return res.status(400).json({
        error: `${original.bookingNumber} allaqachon ${newBookingYear} yilda mavjud`
      });
    }
    ...
    res.status(500).json({ error: 'Nusxa yaratishda xatolik' });
  }
});
